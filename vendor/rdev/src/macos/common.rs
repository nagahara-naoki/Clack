...
pub unsafe fn convert(
    ...
    if let Some(event_type) = option_type {
        // Clack does not use `event.name`, and generating it here queries the
        // current input source through HIToolbox on the event-tap thread.
        // On newer macOS releases that can assert because the API expects the
        // main queue. Returning `None` keeps the physical key event intact
