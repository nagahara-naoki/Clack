...
(function () {
  ...
  function buildHeatmap(entries, period, today) {
    ...
    const calGrid = (grid.closest(".cal-grid") || grid.parentNode) as HTMLElement;
  ...
  function applyPeriodSelection(months: number) {
    C.state.monthsShown = clampMonths(months);
    const pos = (POS_TO_MONTHS as readonly number[]).indexOf(C.state.monthsShown);
  ...
  function selectPeriodByPos(pos) {
    ...
    if (m === C.state.monthsShown) return;
  ...
  function setupPeriodSlider() {
    for (const btn of document.querySelectorAll(".cr-range button[data-pos]")) {
      btn.addEventListener("click", (ev) => {
        if (!ev.isTrusted || !C.navReady()) return;
  ...
  function setupCalNav() {
    ...
    $("cal-next").addEventListener("click", (ev) => {
      if (!ev.isTrusted || !C.navReady()) return;
      if (C.state.periodOffset >= 0) return;
      C.state.periodOffset += 1;
      C.refresh();
    });
  }
  ...
  function setupDayPopover() {
    ...
    function render(detail) {
      ...
      if (d && dateEl) dateEl.textContent = L().dateLabel(d);
      if (d && dowEl) {
      ...
      if (keysEl) keysEl.textContent = fmtNum(detail.keys);
      if (mouseEl) mouseEl.textContent = fmtNum(detail.mouse);
      if (activeEl) activeEl.textContent = L().day_active_fmt(safeInt(detail.activeMs));
      ...
      if (isEmpty) {
        ...
        if (keysListEl) keysListEl.replaceChildren();
        if (mouseListEl) mouseListEl.replaceChildren();
      } else {
    ...
    async function openForCell(cell) {
      ...
      if (cell.classList.contains("is-future") || cell.classList.contains("is-out")) return;
    ...
    document.addEventListener("click", (ev) => {
      ...
      // ポップオーバー外のクリックで閉じる
      if (!popover.hidden && !popover.contains(ev.target as Node | null)) {
        close();
      }
    });
