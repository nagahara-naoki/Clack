...
(function () {
  ...
  function formatTitleJa(start: Date, end: Date): string {
  ...
  function formatTitleEn(start: Date, end: Date): string {
    const sy = start.getFullYear(),
      sm = start.getMonth();
    const ey = end.getFullYear(),
      em = end.getMonth();
  ...
  const I18N: Record<Language, Record<string, any>> = {
    ja: {
      today: "本日",
      keys: "キー",
      mouse: "マウス",
      ...
      hero_delta: (pct: number) => {
        if (!Number.isFinite(pct)) return "—";
        const sign = pct >= 0 ? "+" : "−";
        return `${sign}${Math.abs(pct).toFixed(1)}%`;
      },
      hero_now_at: (h: number, m: number) => `· ${pad2(h)}:${pad2(m)} 時点`,
      trip_goal_to: (name: string) => `${name}まで`,
      trip_goal_passed: (name: string) => `${name} 突破`,
      cal_active_days: (n: number) => (n > 0 ? `· ${fmtNum(n)} 日アクティブ` : ""),
      cal_daily_label: (months: number) => `日次合計 · 過去 ${months} ヶ月`,
      cal_daily_max: (n: number) => `最大 ${fmtNum(n)} / 日`,
      cal_period_totals: "期間合計",
      list_today_badge: "今日",
      list_month_summary: (days: number, keys: number, mouse: number) =>
      ...
      hourTooltip: (hour: number, val: string) => `${hour}:00–${hour}:59 ・ ${val}`,
      ...
      trip_to: (name: string, pct: number | string) => `${name}まで ${pct}%`,
      trip_passed: (name: string) => `${name}を突破`,
      trip_far_beyond: (name: string, mult: number | string) => `${name}の ${mult} 倍`,
      trip_zero: "まだ動いていません",
      today_active: "アクティブ",
      today_scroll: "スクロール",
      today_moved: "移動",
      ...
      dateLabel: (d: Date) => `${d.getMonth() + 1}月${d.getDate()}日 (${DOW_JA[d.getDay()]})`,
      title: (start: Date, end: Date) => formatTitleJa(start, end),
      tooltip: (k: string, m: string) => `キー ${k} ・ マウス ${m}`,
      listDate: (d: Date) => `${d.getMonth() + 1}/${pad2(d.getDate())} (${DOW_JA[d.getDay()]})`,
    },
    en: {
      today: "TODAY",
      keys: "KEYS",
      ...
      hero_now_at: (h: number, m: number) => `· as of ${pad2(h)}:${pad2(m)}`,
      trip_goal_to: (name: string) => `to ${name}`,
      trip_goal_passed: (name: string) => `Passed ${name}`,
      cal_active_days: (n: number) => (n > 0 ? `· ${fmtNum(n)} active days` : ""),
      cal_daily_label: (months: number) => `Daily total · last ${months} months`,
      cal_daily_max: (n: number) => `Max ${fmtNum(n)} / day`,
      ...
      trip_to: (name: string, pct: number | string) => `${pct}% of the way to ${name}`,
      trip_passed: (name: string) => `Passed ${name}`,
      trip_far_beyond: (name: string, mult: number | string) => `${mult}× ${name}`,
      ...
      dowAxis: ["", "mon", "", "wed", "", "fri", ""],
      months: MONTHS_EN,
      dateLabel: (d: Date) => `${MONTHS_EN[d.getMonth()]} ${d.getDate()} (${DOW_EN[d.getDay()]})`,
      title: (start: Date, end: Date) => formatTitleEn(start, end),
      tooltip: (k: string, m: string) => `Keys ${k} · Mouse ${m}`,
      listDate: (d: Date) =>
  ...
  function applyLanguage(lang: string): void {
    const v: Language = lang === "en" ? "en" : "ja";
    if (C.state) C.state.lang = v;
