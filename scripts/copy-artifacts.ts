 ...
 *
 * Source target-dir:
 *   - On Windows, scripts/build.ts sets CARGO_TARGET_DIR to an ASCII path
...
function copyIfExists(src: string, dest: string): boolean {
  ...
  try {
    ...
    // Windows で対象ファイルが実行中だと EBUSY / EPERM になる。
    // バンドル全体を失敗させず、当該ファイルだけ skip して他のコピーを続行。
    if (e.code === "EBUSY" || e.code === "EPERM") {
      console.warn(
        `  [skip] ${path.relative(projectRoot, dest)} is locked (likely running). Close it and re-run \`npm run collect\`.`,
      );
      return false;
    }
    throw err;
...
function copyGlob(srcDir: string, destDir: string, predicate: (name: string) => boolean): number {
...
function main(): void {
  ...
  if (!fs.existsSync(releaseDir)) {
    console.error(`No build output at ${releaseDir}. Run \`npm run build\` first.`);
    process.exit(1);
  }
  ...
  if (isWindows) {
    ...
    if (
      ...
      copied++;
    // Bundled installers.
    copied += copyGlob(path.join(releaseDir, "bundle", "nsis"), distDir, (n) =>
      n.toLowerCase().endsWith(".exe"),
    );
    copied += copyGlob(path.join(releaseDir, "bundle", "msi"), distDir, (n) =>
      n.toLowerCase().endsWith(".msi"),
    );
  } else if (isMac) {
    ...
    // Linux best-effort.
    copied += copyGlob(path.join(releaseDir, "bundle", "deb"), distDir, (n) =>
      n.toLowerCase().endsWith(".deb"),
    );
    copied += copyGlob(path.join(releaseDir, "bundle", "appimage"), distDir, (n) =>
