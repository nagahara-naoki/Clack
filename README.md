...
> 署名 / 公証なしのため、`.app` をダブルクリックすると「Apple はマルウェアが含まれていないことを検証できませんでした」と出ることがあります。まず `Clack.app` を Applications に入れ、**右クリック → 開く** を試してください。`完了` しか出ない場合は、システム設定 → **プライバシーとセキュリティ** の下部に出る **このまま開く** を押してから再度起動してください。
> その後、システム設定 → **プライバシーとセキュリティ → 入力監視** で Clack を ON にしてください。キーボードのグローバル監視にはこの権限が必要です。

### macOS インストール手順（詳細）

1. `.dmg` をダウンロードして開き、`Clack.app` を `Applications` にドラッグ
2. `Clack.app` を **右クリック** → 「開く」を選択（初回のみ）
3. 「検証できませんでした」と表示された場合は「完了」を押す
4. システム設定 → プライバシーとセキュリティを開き、下部に表示される `Clack` の **このまま開く** を押す
5. もう一度 `Clack.app` を開く
6. 入力監視で `Clack` を ON
7. アプリを再起動して完了

それでも開けない場合は、ターミナルで quarantine 属性を外してから起動できます。
...
入力監視を ON にしてもキーボードだけカウントされない場合は、古い Clack の権限情報が macOS に残っている可能性があります。いったん Clack を終了し、システム設定の `入力監視` から Clack を削除してから、次を実行して許可をリセットしてください。
...
起動後に、`入力監視` で Clack を ON にし、アプリを再起動してください。
