...
6. 入力監視で `Clack` を ON
7. 必要に応じて、同じ画面のアクセシビリティでも `Clack` を ON
8. アプリを再起動して完了
...
権限を ON にしても毎回プロンプトが出る、またはキーボードだけカウントされない場合は、古い Clack の権限情報が macOS に残っている可能性があります。いったん Clack を終了し、システム設定の `入力監視` と `アクセシビリティ` から Clack を削除してから、次を実行して許可をリセットしてください。
...
起動後に、`入力監視` と `アクセシビリティ` の両方で Clack を ON にし、アプリを再起動してください。
