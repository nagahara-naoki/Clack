...
(() => {
    ...
    function setTodayCounts(payload) {
        ...
        const tk = $("today-keys");
        if (tk)
            tk.textContent = fmtNum(keys);
        const tm = $("today-mouse");
        if (tm)
            tm.textContent = fmtNum(mouse);
        // 分析ヒーロー: アニメ中ならスキップ、それ以外は直更新。
        const heroKeys = $("hero-keys-num");
        if (heroKeys && !heroKeys._animRaf)
            heroKeys.textContent = fmtNum(keys);
        const heroMouse = $("hero-mouse-num");
        if (heroMouse && !heroMouse._animRaf)
            heroMouse.textContent = fmtNum(mouse);
        const d = parseDate(payload.date);
        if (d) {
            const td = $("today-date");
            if (td)
                td.textContent = L().dateLabel(d);
        }
    }
    ...
    async function refresh() {
        ...
        if (calDate)
            calDate.textContent = dateStr;
        const listDate = $("list-date-str");
        if (listDate)
            listDate.textContent = dateStr;
        // タイトル更新 (cr-cal-h のメインタイトル)
        const titleEl = $("cal-title");
        if (titleEl)
            titleEl.textContent = L().title(period.start, period.end);
        // 範囲データ取得
        let data;
        try {
        ...
        if (activeEl)
            activeEl.textContent = L().cal_active_days(activeDays);
        // ---- 期間合計 (キー / マウス) ----------------------------------------
        const tKeys = $("cal-total-keys");
        const tMouse = $("cal-total-mouse");
        if (tKeys)
            C.analytics.animateCountUp(tKeys, totalKeys);
        if (tMouse)
            C.analytics.animateCountUp(tMouse, totalMouse);
        // ---- 日次合計 sparkline (棒チャート) --------------------------------
        C.calendar.renderDailySpark(dailyTotals);
        const dailyLabelEl = $("cal-daily-label");
        if (dailyLabelEl)
            dailyLabelEl.textContent = L().cal_daily_label(state.monthsShown);
    ...
    function setupTooltip() {
        ...
        const setPos = (rect) => {
            ...
            if (top < margin)
                top = Math.min(rect.bottom + 6, vh - tipH - margin);
