...
(function () {
    ...
    async function fetchHeroSeries() {
        ...
        try {
            ...
            for (const r of rows) {
                if (r?.date)
                    byDate.set(r.date, { keys: safeInt(r.keys), mouse: safeInt(r.mouse) });
            }
            const keys = [];
            const mouse = [];
    ...
    function renderHeroKPI({ prefix, todayVal, avg7Val, series }) {
        ...
        if (numEl)
            animateCountUp(numEl, todayVal);
        if (avg7El)
            avg7El.textContent = fmtNum(Math.round(avg7Val));
        if (deltaEl) {
    ...
    function renderStripTrip({ prefix, meters, landmarks }) {
        ...
        if (!numEl || !unitEl || !goalEl || !fillEl || !pctEl)
            return;
    ...
    function updateAnalyticsSubhead() {
        ...
        if (dateEl)
            dateEl.textContent = L().dateLabel(now);
        const clockEl = $("cr-clock-str");
        if (clockEl)
            clockEl.textContent = L().hero_now_at(now.getHours(), now.getMinutes());
    }
    ...
    async function refresh() {
        ...
        const avg7Hourly = (Array.isArray(weekData.hourly) ? weekData.hourly : new Array(24).fill(0)).map((v) => Math.max(0, Number(v) || 0) / 7);
    ...
    function setupScope() {
        for (const btn of document.querySelectorAll("[data-scope]")) {
            btn.addEventListener("click", (ev) => {
                ...
                if (next === C.state.analyticsScope)
                    return;
