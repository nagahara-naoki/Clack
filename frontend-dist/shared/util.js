...
(function () {
    ...
    const KEY_LABEL_MAP = IS_MAC ? KEY_LABEL_MAP_MAC : KEY_LABEL_MAP_WIN;
    ...
    function prettyKey(label) {
        if (!label)
            return "";
        if (KEY_LABEL_MAP[label])
            return KEY_LABEL_MAP[label];
        if (/^Key[A-Z]$/.test(label))
            return label.slice(3);
        if (/^Num\d$/.test(label))
            return label.slice(3);
        if (/^Kp\d$/.test(label))
            return `Num${label.slice(2)}`;
        if (/^F\d{1,2}$/.test(label))
            return label;
        return label;
    }
    // 公開
